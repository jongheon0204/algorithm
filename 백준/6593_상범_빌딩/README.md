...
상범 빌딩은 높이가 L이며 한 층마다 R \* C 크기라 할 때 시작 지점인 S에서 탈출 지점인 E로 가는데 걸리는 최소 시간을 구하는 문제이다.

최소 거리를 찾는 문제는 BFS 방법을 통해 쉽게 구할 수 있다
        ...
        while(true){
            ...
            while(!que.isEmpty()){
                ...
                for(int i=0; i<6; i++){
                    ...
                    if(!isRange(next[0], next[1], next[2]) || dis[next[0]][next[1]][next[2]] == -1) continue;
                    if(dis[next[0]][next[1]][next[2]] <= dis[cur[0]][cur[1]][cur[2]] + 1) continue;
